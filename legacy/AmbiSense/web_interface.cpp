...
extern void getESPNOWMacAddress(uint8_t* mac_addr);
...
<h2>🔬 Real-Time System Diagnostics</h2>
<div style='display:flex;align-items:center;justify-content:center;margin-top:10px'>
<div style='width:8px;height:8px;border-radius:50%;background:var(--success);box-shadow:0 0 6px var(--success);animation:pulse 2s infinite'></div>
...
<div class='metric'>
<span class='metric-label'>ESP-NOW MAC</span>
<span class='metric-value' id='espnowMacDisplay'>Loading...</span>
</div>
<div class='metric'>
...
<div style='position:absolute;top:10px;left:10px;font-size:12px;color:var(--text-secondary)'>Distance: <span id='chartDistance'>--</span> cm</div>
  ...
  // JavaScript for diagnostics functionality
  String diagnosticsScripts = F(R"(
<script>
function updateDiagnostics() {
  fetch('/diagnosticdata')
    ...
    .then(data => {
      ...
      if (data.espnowMac) document.getElementById('espnowMacDisplay').textContent = data.espnowMac;
...
void setupWebServer() {
  ...
  server.on("/wizard/layout", HTTP_POST, handleWizardStep);  // Alias for layout step
  server.on("/wizard/discover", HTTP_POST, handleWizardDiscover);
  server.on("/wizard/validate", HTTP_GET, handleWizardValidate);
  server.on("/wizard/apply", HTTP_POST, handleWizardApply);
...
void handleNetworkPost() {
  if (server.hasArg("ssid") && server.hasArg("deviceName")) {
    ...
    html += ".spinner{width:50px;height:50px;border:5px solid rgba(255,255,255,0.1);border-radius:50%;border-top-color:#4361ee;animation:spin 1s ease-in-out infinite;margin:20px auto;}";
    html += "@keyframes spin{to{transform:rotate(360deg);}}";
    html += ".success{color:#4cc9f0;}";
    html += ".error{color:#ff6b6b;}";
    html += "</style></head><body>";
    html += "<h1>✅ WiFi Settings Saved</h1>";
    html += "<div class=\"spinner\"></div>";
    html += "<p id=\"status\">Device is restarting and connecting to new network...</p>";
    html += "<p>New device address: <strong id=\"hostname\">http://" + hostname + ".local</strong></p>";
    html += "<div id=\"countdown\" style=\"margin-top:30px;font-size:14px;color:#b0b0b0;\"></div>";
    html += "<script>";
    html += "const hostname = '" + hostname + "';";
    html += "let attempts = 0;";
    html += "let maxAttempts = 30;";
    html += "let redirectUrl = `http://${hostname}.local`;";
    html += "function updateStatus(message, isError = false) {";
    html += "document.getElementById('status').innerHTML = message;";
    html += "document.getElementById('status').className = isError ? 'error' : '';";
    html += "}";
    html += "function updateCountdown(seconds) {";
    html += "document.getElementById('countdown').innerHTML = `Checking connection... (${seconds}s remaining)`;";
    html += "}";
    html += "function checkConnection() {";
    html += "attempts++;";
    html += "const remainingTime = Math.max(0, (maxAttempts - attempts) * 2);";
    html += "updateCountdown(remainingTime);";
    html += "if (attempts > maxAttempts) {";
    html += "updateStatus('⚠️ Auto-redirect failed. Please manually visit:<br><strong>' + redirectUrl + '</strong>', true);";
    html += "document.getElementById('countdown').innerHTML = '<a href=\"' + redirectUrl + '\" style=\"color:#4361ee;\">Click here to access your device</a>';";
    html += "return;";
    html += "}";
    html += "const controller = new AbortController();";
    html += "const timeoutId = setTimeout(() => controller.abort(), 3000);";
    html += "fetch(redirectUrl + '/', {";
    html += "method: 'HEAD',";
    ...
    html += "updateStatus('🎉 Connection successful! Redirecting...');";
    html += "setTimeout(() => {";
    ...
    html += "};";
    html += "img.onerror = () => {";
    html += "setTimeout(checkConnection, 2000);";
    html += "};";
    html += "img.src = redirectUrl + '/favicon.ico?' + Date.now();";
    html += "});";
    html += "}";
    html += "setTimeout(() => {";
    html += "updateStatus('🔍 Checking if device is accessible at new address...');";
...
void handleScanForSlaves() {
  ...
  for (int i = 0; i < numDevices && devicesFound < 10; i++) {
    ...
    if (isAmbiSense && ssid.length() > 0) {
      ...
      if (sta_mac[5] > 0) {
        sta_mac[5]--; // Convert AP MAC to STA MAC for ESP-NOW
      } else {
...
void handleWizardStep() {
  // Use the fixed wizard implementation for wizard steps
...
void handleWizardValidate() {
  // Use the fixed wizard implementation for validation
  bool isValid = validateWizardConfiguration();
