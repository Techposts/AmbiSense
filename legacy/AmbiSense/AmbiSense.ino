...
void checkForFactoryReset() {
  // Check if reset button is held during boot (LOW when pressed with INPUT_PULLUP)
...
void setup() {
  ...
  // Set device role for WiFi manager before initialization
  wifiManager.setDeviceRole(deviceRole);
  
  // Initialize WiFi AFTER other hardware is set up
  Serial.println("Starting WiFi manager...");
  wifiManager.begin();
  Serial.printf("WiFi AP IP after WiFi init: %s\n", WiFi.softAPIP().toString().c_str());
  
  // Initialize ESP-NOW for master-slave communication
  Serial.println("Starting ESP-NOW...");
  setupESPNOW();
  Serial.printf("WiFi AP IP after ESP-NOW init: %s\n", WiFi.softAPIP().toString().c_str());
  
  // Setup Web server LAST after WiFi is connected
  Serial.println("Starting Web server...");
  setupWebServer();
  Serial.printf("WiFi AP IP after Web server init: %s\n", WiFi.softAPIP().toString().c_str());
...
void loop() {
  ...
  // Check if WiFi reset button is held for a long time (LOW when pressed with INPUT_PULLUP)
