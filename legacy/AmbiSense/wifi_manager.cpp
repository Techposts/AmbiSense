...
void WifiManager::begin() {
  ...
  // Role-based WiFi behavior
  if (_deviceRole == 2) { // Slave role
    Serial.println("[WiFi] Slave device - starting in AP mode only");
    startAPMode();
    _currentMode = WIFI_MANAGER_MODE_AP;
  } else { // Master role
    // Try to load saved credentials
    if (loadWifiCredentials(ssid, password, deviceName) && strlen(ssid) > 0) {
      Serial.printf("[WiFi] Master device - saved credentials found: SSID='%s'\n", ssid);
      ...
      if (connectToWifi(ssid, password)) {
        ...
        // For masters, switch to STA-only mode after successful connection
        switchToSTAOnly();
      } else {
...
bool WifiManager::startAPMode() {
  ...
  // Use a more unique AP name based on chip ID and timestamp
  char apName[32];
  uint32_t chipId = ESP.getEfuseMac() & 0xFFFF; // Use last 16 bits (4 hex digits)
  sprintf(apName, "AmbiSense-%04X", chipId);
  ...
  // Use ESPNOW_CHANNEL for consistent AP and ESP-NOW communication
  int channel = ESPNOW_CHANNEL;  // Use same channel as ESP-NOW for compatibility
  Serial.printf("[WiFi] Using channel %d for AP mode (ESP-NOW compatible)\n", channel);
  
  // Configure WiFi power and performance
  WiFi.setTxPower(WIFI_POWER_19_5dBm); // Max power for better range
  
  // Start AP with ESP-NOW compatible channel
  bool success = WiFi.softAP(apName, WIFI_AP_PASSWORD, channel, 0, 4);  // ESP-NOW channel, not hidden, max 4 clients
...
void WifiManager::process() {
  ...
  // ESP32 mDNS doesn't require manual updates like ESP8266
  // mDNS automatically handles updates in ESP32
...
void WifiManager::setDeviceRole(uint8_t role) {
  _deviceRole = role;
  Serial.printf("[WiFi] Device role set to: %s\n", 
               (role == 1) ? "Master" : "Slave");
}

void WifiManager::switchToSTAOnly() {
  if (_currentMode == WIFI_MANAGER_MODE_STA && _deviceRole == 1) { // Master role
    Serial.println("[WiFi] Master connected to WiFi - keeping AP+STA mode for ESP-NOW compatibility");
    
    // Keep AP+STA mode for ESP-NOW compatibility
    // Don't switch to STA-only as it breaks ESP-NOW
    WiFi.mode(WIFI_AP_STA);
    
    Serial.println("[WiFi] Maintaining AP+STA mode for ESP-NOW");
  }
}