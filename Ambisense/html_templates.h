...
// Network Settings HTML Template
const char networkSettingsTemplate[] PROGMEM = R"rawliteral(
<!DOCTYPE html>
<html>
<head>
  <title>Network Settings - AmbiSense</title>
...
// Main HTML Template
const char mainHtmlTemplate[] PROGMEM = R"rawliteral(
<!DOCTYPE html>
<html lang="en">
<head>
...
<!-- Color picker section -->
          ...
          <style>
            /* CSS for the color picker implementation */
  ...
  <script>
    ...
    function loadCurrentSettings() {
      fetch('/settings')
        ...
        .then(settings => {
          ...
          document.getElementById("predictionFactor").value = settings.predictionFactor * 100;
          document.getElementById("predictionValue").textContent = settings.predictionFactor.toFixed(2);
          
          document.getElementById("positionPGain").value = settings.positionPGain * 100;
          document.getElementById("positionPGainValue").textContent = settings.positionPGain.toFixed(2);
          
          document.getElementById("positionIGain").value = settings.positionIGain * 1000;
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function updateRGBFromHex(hexColor) {
        ...
        // Save the RGB values
        autoSaveChanges('redValue', r);
        autoSaveChanges('greenValue', g);
        autoSaveChanges('blueValue', b);
      }
