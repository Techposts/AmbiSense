...
import { ScreenLive, ScreenLeds, ScreenMotion, ScreenPresence, ScreenHardware, ScreenNetwork, ScreenSystem } from './screens';
import './styles.css';

type Tab = 'live'|'leds'|'motion'|'presence'|'hardware'|'network'|'system';

const TABS: { id: Tab; name: string; icon: string }[] = [
  { id: 'live',     name: 'Live',     icon: 'dashboard' },
  { id: 'leds',     name: 'LEDs',     icon: 'led' },
  { id: 'motion',   name: 'Motion',   icon: 'motion' },
  { id: 'presence', name: 'Presence', icon: 'person' },
...
function App() {
  ...
  const [live, setLive] = useState<any>({ distance: 0, raw: 0, direction: 0, rssi: 0, heap: 0, uptime: 0, occupied: false, count: 0, stationary: false, nearest_cm: -1, seconds_since_seen: 0 });
  ...
  const switcher: Record<Tab, any> = { live: ScreenLive, leds: ScreenLeds, motion: ScreenMotion, presence: ScreenPresence, hardware: ScreenHardware, network: ScreenNetwork, system: ScreenSystem };
