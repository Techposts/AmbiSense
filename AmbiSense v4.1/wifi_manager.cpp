...
#define MAX_CONNECTION_ATTEMPTS 3      // Number of initial connection attempts before fallback to AP mode
...
void WifiManager::begin() {
  ...
  if (loadWifiCredentials(ssid, password, deviceName) && strlen(ssid) > 0) {
    ...
    // Try to connect up to MAX_CONNECTION_ATTEMPTS times
    ...
    while (!connected && _connectionAttempts < MAX_CONNECTION_ATTEMPTS) {
      ...
      if (connectToWifi(ssid, password)) {
        ...
        Serial.println("[WiFi] Successfully connected to WiFi network");
        ...
        if (_connectionAttempts < MAX_CONNECTION_ATTEMPTS) {
          Serial.printf("[WiFi] Waiting 30 seconds before next attempt...\n");
    ...
    if (!connected) {
      Serial.printf("[WiFi] Failed to connect after %d attempts. Starting AP mode...\n", MAX_CONNECTION_ATTEMPTS);
      startAPMode();
...
bool WifiManager::startAPMode() {
  ...
  // Create a unique AP name using chip ID
  char apName[64];
  uint32_t chipId = ESP.getEfuseMac() & 0xFFFFFFFF;
  ...
  if (loadWifiCredentials(ssid, password, deviceName) && strlen(deviceName) > 0) {
    sprintf(apName, "AmbiSense-%s-Setup", deviceName);
  } else {
    sprintf(apName, "AmbiSense-%08X-Setup", chipId);
  }
  
  WiFi.mode(WIFI_AP);
  bool success = WiFi.softAP(apName, WIFI_AP_PASSWORD);
  
  if (success) {
    Serial.print("[WiFi] AP started successfully. Name: ");
    Serial.println(apName);
    Serial.print("[WiFi] AP IP Address: ");
    Serial.println(WiFi.softAPIP());
  } else {
    Serial.println("[WiFi] Failed to start AP! Using default name as fallback.");
    // Try with the default name as fallback
    success = WiFi.softAP(WIFI_AP_SSID, WIFI_AP_PASSWORD);
    if (success) {
      Serial.print("[WiFi] Default AP started. Name: ");
      Serial.println(WIFI_AP_SSID);
      Serial.print("[WiFi] AP IP Address: ");
      Serial.println(WiFi.softAPIP());
    } else {
      Serial.println("[WiFi] Critical failure: Could not start AP mode!");
    }
  }
...
String WifiManager::getSanitizedHostname(const char* deviceName) {
  ...
  if (deviceName && strlen(deviceName) > 0) {
    ...
    // First, check if it already starts with the prefix
...
void WifiManager::process() {
  ...
  static bool ledState = HIGH;  // Start as OFF
  ...
  if (_currentMode == WIFI_MANAGER_MODE_STA && WiFi.status() == WL_CONNECTED) {
    ...
    switch (blinkPhase) {
      case 0:
        digitalWrite(WIFI_STATUS_LED_PIN, LOW); // ON
        lastBlinkTime = now;
        blinkPhase++;
        break;
      case 1:
        if (now - lastBlinkTime >= 100) {
          digitalWrite(WIFI_STATUS_LED_PIN, HIGH); // OFF
          lastBlinkTime = now;
          blinkPhase++;
        }
        break;
      ...
      case 3:
        ...
        break;
      case 4:
        if (now - lastBlinkTime >= 4600) { // Rest of the 5s cycle
          blinkPhase = 0;
        }
        break;
    }
  } else {
    // Fast blink for AP/Fallback mode
    if (now - lastBlinkTime >= 200) {
      lastBlinkTime = now;
      ledState = !ledState;
      digitalWrite(WIFI_STATUS_LED_PIN, ledState ? LOW : HIGH); // Inverted toggle
    }
  }

  // Periodic Wi-Fi reconnect check
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (loadWifiCredentials(ssid, password, deviceName)) {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        _connectionAttempts = 0;
        _currentMode = WIFI_MANAGER_MODE_STA; // Ensure mode is set correctly
      } else {
...
void WifiManager::updateLedStatus() {
  // Already handled in the process method
}
