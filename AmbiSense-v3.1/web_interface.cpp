...
<!-- Completely revised color picker for cross-platform compatibility -->
  ...
  <script>
    ...
    document.getElementById('saveButton').addEventListener('click', function() {
      ...
      xhr.onload = function() {
        if (xhr.status === 200) {
          // Also save advanced settings
          const centerShift = document.getElementById('centerShift').value;
          const trailLength = document.getElementById('trailLength').value;
          
          // Save center shift
          fetch('/setCenterShift?value=' + centerShift)
            .then(() => {
              // Save trail length
              return fetch('/setTrailLength?value=' + trailLength);
            })
            .then(() => {
              // Show notification when all settings have been saved
...
void handleNetworkSettings() {
  if (server.hasArg("ssid") && server.hasArg("password") && server.hasArg("deviceName")) {
    ...
    if (wifiManager.getMode() == WIFI_MANAGER_MODE_STA) {
      mdnsHtml = "<div class=\"status-item\"><span class=\"status-label\">mDNS Name:</span><span class=\"status-value\">http://" + 
