...
/**
 * Get the correct ESP-NOW MAC address for this device
 * @param mac_addr Buffer to store the 6-byte MAC address
 */
void getESPNOWMacAddress(uint8_t* mac_addr);

/**
 * Enhanced zone-based priority for L/U-shaped stairs
 * @param currentTime Current timestamp
 * @param stairLayout Layout type (0=straight, 1=L-shape, 2=U-shape)
 */
int handleEnhancedZonePriority(unsigned long currentTime, int stairLayout);

/**
 * Smart corner transition handling for L/U stairs
 * @param sensorReadings Array of current sensor readings
 * @param numSensors Number of active sensors
 * @param layout Stair layout type
 */
int smartCornerTransition(int sensorReadings[], int numSensors, int layout);

/**
 * Calculate optimal LED position for complex layouts
 * @param selectedDistance Distance from active sensor
 * @param activeSensorId ID of currently active sensor
 * @param layout Stair layout type
 */
int calculateComplexLEDPosition(int selectedDistance, int activeSensorId, int layout);
