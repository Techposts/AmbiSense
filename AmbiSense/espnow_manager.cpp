...
// sensorPriorityMode is defined in AmbiSense.ino as extern in config.h
...
void setupESPNOW() {
  ...
  // Store current WiFi state for restoration if needed
  wifi_mode_t currentMode = WiFi.getMode();
  String currentAPSSID = WiFi.softAPSSID();
  IPAddress currentAPIP = WiFi.softAPIP();
  bool hadAP = (currentMode == WIFI_AP || currentMode == WIFI_AP_STA) && (currentAPIP != IPAddress(0,0,0,0));

  Serial.printf("ESP-NOW: Current WiFi mode: %d, AP Active: %s\n", currentMode, hadAP ? "Yes" : "No");

  // Force AP+STA mode for ESP-NOW compatibility
  if (currentMode != WIFI_AP_STA) {
    Serial.println("ESP-NOW: Switching to AP+STA mode for ESP-NOW compatibility");
    WiFi.mode(WIFI_AP_STA);
    delay(300);  // Longer delay for mode stabilization

    // Restore AP if it was running before
    if (hadAP && currentAPSSID.length() > 0) {
      Serial.printf("ESP-NOW: Restoring AP: %s\n", currentAPSSID.c_str());
      bool apResult = WiFi.softAP(currentAPSSID.c_str(), WIFI_AP_PASSWORD, ESPNOW_CHANNEL, 0, 4);
      if (apResult) {
        Serial.printf("ESP-NOW: AP restored successfully: %s\n", WiFi.softAPIP().toString().c_str());
      } else {
        Serial.println("ESP-NOW: WARNING - Failed to restore AP");
      }
    }
  } else {
    Serial.println("ESP-NOW: WiFi already in AP+STA mode");
  }

  // Set fixed channel for consistent communication, but preserve AP if it exists
  int currentChannel = WiFi.channel();
  Serial.printf("ESP-NOW: Current WiFi channel: %d, target: %d\n", currentChannel, ESPNOW_CHANNEL);
  ...
  // Get and print ESP-NOW MAC address (different from WiFi MAC)
  ...
  Serial.println(espnow_mac_str);
  Serial.printf("ESP-NOW: Using channel %d\n", ESPNOW_CHANNEL);

  // Also show WiFi MAC for comparison
  Serial.printf("WiFi MAC (different): %s\n", WiFi.macAddress().c_str());
  ...
  // Verify ESP-NOW channel and MAC configuration
  Serial.printf("ESP-NOW: Actual WiFi channel: %d\n", WiFi.channel());
  if (WiFi.channel() != ESPNOW_CHANNEL) {
    Serial.printf("ESP-NOW: WARNING - WiFi channel mismatch! Expected: %d, Actual: %d\n",
  ...
  if (result != ESP_OK && ENABLE_ESPNOW_LOGGING) {
    ...
    if (millis() - lastErrorTime > 1000) {
      ...
      sprintf(masterMacStr, "%02X:%02X:%02X:%02X:%02X:%02X",
              masterAddress[0], masterAddress[1], masterAddress[2],
              masterAddress[3], masterAddress[4], masterAddress[5]);
      Serial.printf("ESP-NOW: Failed to send data to %s after %d retries (error: %d)\n",
                   masterMacStr, ESPNOW_RETRY_COUNT, result);
...
// Helper function to get correct ESP-NOW MAC address
void getESPNOWMacAddress(uint8_t* mac_addr) {
  esp_wifi_get_mac(WIFI_IF_STA, mac_addr);  // ESP-NOW uses STA MAC as base
}