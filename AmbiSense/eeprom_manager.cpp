...
void validateCriticalSettings() {
  ...
  if (settingsChanged) {
    ...
    // Commit changes immediately for critical fixes
    EEPROM.commit();
...
void saveSettings() {
  ...
  // Commit with robust retry mechanism
  bool committed = false;
  int retryCount = 0;
  const int maxRetries = 3;
  const int retryDelays[] = {50, 150, 300}; // Progressive delays
  
  while (!committed && retryCount <= maxRetries) {
    if (retryCount > 0) {
      Serial.printf("EEPROM commit attempt %d/%d...\n", retryCount + 1, maxRetries + 1);
    ...
    if (committed) {
      if (retryCount == 0) {
        ...
        Serial.printf("EEPROM commit succeeded on retry %d\n", retryCount);
      }
      ...
      if (retryCount <= maxRetries) {
        Serial.printf("EEPROM commit failed (attempt %d/%d), retrying...\n", retryCount, maxRetries + 1);
      } else {
        Serial.println("CRITICAL: EEPROM commit failed after all retries!");
        Serial.println("Settings may not persist after restart. Consider factory reset.");
      }
    }
  }
}
...
void saveAdvancedSettings() {
  ...
  // Note: commit handled by main saveSettings() function
}
...
uint8_t calculateSystemCRC() {
  ...
  for (int i = EEPROM_SYSTEM_START; i < EEPROM_ADVANCED_START; i++) {
    if (i != EEPROM_ADDR_CRC) { // Skip CRC address itself for CRC calculation
      crc ^= EEPROM.read(i);
    }
  }
